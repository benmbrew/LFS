plot_models_performance <- function(models, num.of.partition, features.name, pdf.name, plot.title) {
  ...
  boxplot_matrix_mse <- foreach (temp.ind = 1:num.of.partition, .combine=rbind) %do% {
    c(
      mean(as.numeric(models[[temp.ind]]$elasticNet))
      , mean(as.numeric(models[[temp.ind]]$lasso))
      , mean(as.numeric(models[[temp.ind]]$ridge))
      , mean(as.numeric(models[[temp.ind]]$rf))
      , mean(as.numeric(models[[temp.ind]]$dtree))
      , mean(as.numeric(models[[temp.ind]]$svmLinear))
      , mean(as.numeric(models[[temp.ind]]$svmRadial))
    )  
  }
...
#     )  
#   }
  temp.boxplot_data_mse <- data.frame(boxplot_matrix_mse)
...
#   }
  ...
  boxplot(temp.boxplot_data_mse, las=2,
          main=plot.title,
          par(mar = c(10, 5, 4, 2) + 0.1), 
          ylab = "MSE on test set",
          names = temp.names,
          at = rank(apply(temp.boxplot_data_mse, 2, mean), ties.method = c("first"))
  )
