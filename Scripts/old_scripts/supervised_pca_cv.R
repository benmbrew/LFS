...
train_test_pca <- 
  function(cases,
    ...
    for (i in 1:max(fold_numbers)) {
      ...
      # get scores, number of features, and threshold
      ...
      # first get training pca
      train_pca <- superpc.predict(train_object, 
                                  train_dat, 
                                  test_dat, 
                                  threshold = opt_thresh[[i]], 
                                  n.components=3, 
                                  prediction.type="continuous")$v.train
      
      
      # will return correlation on test data with pred_pca predictors 
      lm_train <- as.data.frame(cbind(y = train_dat$y, train_pca))
      
      # build simple linear model - regress y on 3 PCAs and save model
      lm_model[[i]] <- lm(y ~., data = lm_train)
      
      # get test PCAs with optimal threshold 
      test_pca <- superpc.predict(train_object, 
                                  train_dat, 
                                  test_dat, 
                                  threshold = opt_thresh[[i]], 
                                  n.components=3, 
      ...
      # make test_pca a data frame 
      test_pca <- as.data.frame(test_pca)
      colnames(test_pca) <- c('V2', 'V3', 'V4')
      
      # test model built on training data on test data using 
      predict(lm_model[[i]], test_pca)
