...
# ##########
# plot(beta_full$age_diagnosis[beta_full$type == '450k'], beta_full$age_sample_collection[beta_full$type == '450k'])
# temp <- beta_full
# temp_1 <- data.frame(diff = (temp$age_sample_collection - temp$age_diagnosis))
# 
# # histogram of differenc
# ggplot(temp_1, aes(temp_1$diff)) + 
# geom_histogram(bins = 20, alpha=.8, position="identity", colour = 'black', fill = 'red') + 
#   xlab('Gap between diagnosis and sample collection') + ylab('Frequency in clinical data')+ 
#   theme_bw() + theme(axis.text.y = element_text(size = 12),
#                      axis.text.x = element_text(size = 12))
# 
# # indicate over 24 and group by 
# temp <- beta_full[, 1:8]
# temp$diff <- temp$age_sample_collection - temp$age_diagnosis
# temp$diff_fac <- ifelse(temp$diff > 24, 'big', 'small')
# temp_group <- temp %>%
#   filter(!is.na(diff_fac)) %>%
#   group_by(diff_fac) %>%
#   summarise(mean_diagnosis = mean(age_diagnosis, na.rm = T),
#             mean_collection = mean(age_sample_collection, na.rm = T),
#             acc_tot = sum(cancer_diagnosis_diagnoses == 'ACC', na.rm = T),
#             other_tot = sum(cancer_diagnosis_diagnoses != 'ACC', na.rm = T),
#             mean_f = mean(M, na.rm = T))
# 
                      ...
                      k) 
{
  ...
  # create variable to 
  cases <- cases[which((cases$age_sample_collection - cases$age_diagnosis) > diff_thresh),]
...
feature_length <- c(100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000, 3000, 4000, 5000, 10000, 20000)
seeds <- c(1,2,3)
