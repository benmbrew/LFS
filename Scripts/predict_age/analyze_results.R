...
reg_folder <- paste0(results_folder, '/reg_results_30_sig')
...
for (i in 1:iter_length) {
  ...
  for (j in 1:folds){
    ...
    temp.fold_norm_dat$V6 <- 'norm'
    temp.fold_resid_dat$V6 <- 'resid'
    
    

    # get column names 
    colnames(temp.fold_norm_dat) <- c('alpha', 'lambda', 'onset_correlation', 'age_correlation', 'vars', 'type')
    colnames(temp.fold_resid_dat) <- c('alpha', 'lambda', 'onset_correlation', 'age_correlation', 'vars', 'type')
