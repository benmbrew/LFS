...
run_model <- function(cases_full,
  ...
  for(i in 1:k_folds) {
    ...
    # get features (probe names)
    data_features <- colnames(cases_full)[16:ncol(cases_full)]

    # take remove features out of colnames 
    bh_features <- data_features[!data_features %in% remove_features]
    
    # function to predict with all test, controls, controls old, and valid
    mod_result  <- run_glmm_lasso(training_dat = train_cases,
