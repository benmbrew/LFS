  ...
  
{
  ...
  # get features
  if(gender) {
    all_feat <- colnames(data)[c(3,5:ncol(data))]
  ...
  for(i in 1:num_it) {
    ...
    if(gender) {
      #make gender binary 1,2
      dat_train$gender <- ifelse(dat_train$gender == 'M', 1, 0)
      dat_test$gender <- ifelse(dat_test$gender == 'M', 1, 0)
