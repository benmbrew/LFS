...
} else if(type == 'no_transform'){
  new_cols <- colnames(readRDS(paste0(model_data, paste0('/controls_no_transform_union_m','_' , method, '_2.rda'))))
  betaCases <- readRDS(paste0(model_data, paste0('/', method, '_', 'cases_new_m.rda')))[,new_cols]
  betaControls <- readRDS(paste0(model_data, paste0('/controls_no_transform_union_m','_' , method, '_2.rda'))) # 34 435813
  betaValid <- readRDS(paste0(model_data, paste0('/valid_no_transform_union_m','_' , method, '_2.rda')))# 45 400842
  
} else {
  # read in old threshold data to get column names 
  old_cols <- colnames(readRDS(paste0(model_data, paste0('/controls_no_transform_union','_' , 'raw', '_0.05.rda')))) # 34 435813
  # subset 
  # read in full m value data 
  betaCases <- readRDS(paste0(model_data, paste0('/', method, '_', 'cases_new_batch_m.rda')))[, old_cols]
  betaControls <- readRDS(paste0(model_data, paste0('/', method, '_', 'controls_new_m.rda')))[,old_cols ] #34 449936
  betaValid <- readRDS(paste0(model_data, paste0('/', method, '_', 'valid_new_m.rda')))[,old_cols] #35 449783
...
# controls_full <- betaControlsFull
# controls_old <- betaControlsOld
# valid <- betaValid
# use_bh <- T
# mod_feat <- intersect_names
# i = 1
                      ...
                      k) 
{
  ...
  for (i in 1:k) {
    ...
    if (use_bh){
      ...
      mod_feat <- bh_feat_all
    } else {
      bh_dim[[i]] <- length(mod_feat)
    }
    ...
    mod_result <- runEnet(training_dat = cases[train_index,], 
                          ...
                          controls_dat_old = controls_old,
                          controls_dat_full = controls_full,
                          valid_dat = valid,
                          bh_features = mod_feat,
                          gender = T)
