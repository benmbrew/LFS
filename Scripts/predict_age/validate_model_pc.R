...
if(size =='used_bh'){
  ...
  if(method == 'noob'){
    if(!grepl('combat', combat)){
      if(max(cases_450$age_diagnosis) < 100){
        # divide ages by 12 
        cases_450$age_diagnosis <- round((cases_450$age_diagnosis*12), 2)
        cases_450$age_sample_collection <- round((cases_450$age_sample_collection*12), 2)
      ...
      if(max(cases_850$age_diagnosis) < 100){
        # divide ages by 12 
        cases_850$age_diagnosis <- round((cases_850$age_diagnosis*12), 2)
        cases_850$age_sample_collection <- round((cases_850$age_sample_collection*12), 2)
      ...
      if(max(con_mut$age_sample_collection) < 100){
        
        con_mut$age_sample_collection <- round((con_mut$age_sample_collection*12), 2)
  ...
  con_mut <- readRDS(paste0('../../Data/', method,'/con_mut_', combat,'.rda'))
...
if(model_type == 'enet'){
  ...
  # read in cases_450
  saveRDS(temp_con, paste0('pc_data_test/', 'con_test', method,'_',size,'_',is_gen, '_',combat,'_', model_type,'.rda'))
  
  saveRDS(temp_valid, paste0('pc_data_test/', 'valid_test',method,'_',size,'_',is_gen, '_',combat,'_', model_type,'.rda'))
  ...
  saveRDS(temp_con, paste0('pc_data_test/', 'con_test_pc',method,'_',size,'_',is_gen, '_',combat,'_', model_type,'.rda'))
  
  saveRDS(temp_valid, paste0('pc_data_test/', 'valid_test_pc',method,'_',size,'_',is_gen, '_',combat,'_', model_type,'.rda'))
  
  saveRDS(temp_importance, paste0('pc_data_test/', 'importance_pc',method,'_',size,'_',is_gen, '_',combat,'_', model_type,'.rda'))
