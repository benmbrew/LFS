...
# null set
temp_450 <- final_dat_con %>% 
  filter(tech == '450k') %>%
  group_by(tm_donor, seed) %>% 
...
temp_850$real <- as.factor(ifelse(temp_850$age_sample_collection<= 72, 'positive', 'negative'))
temp_850$real <- factor(temp_850$real, levels = c('positive', 'negative'))

temp_450$real <- as.factor(ifelse(temp_450$age_sample_collection<= 72, 'positive', 'negative'))
temp_450$real <- factor(temp_450$real, levels = c('positive', 'negative'))


# valid
final_dat_val$optimal_thresh <- optimal_thresh

final_dat_val <- final_dat_val %>% group_by(tm_donor) %>% summarise(preds = mean(positive),
                                                            age_diagnosis = mean(age_diagnosis),
                                                            age_sample_collection = mean(age_sample_collection),
                                                            optimal_thresh = mean(optimal_thresh),
                                                            counts = n())
final_dat_val$real <- as.factor(ifelse(final_dat_val$age_diagnosis <= 72, 'positive', 'negative'))
final_dat_val$real <- factor(final_dat_val$real, levels = c('positive', 'negative'))
final_dat_val$pred_class <- as.factor(ifelse(final_dat_val$preds > .5, 'positive', 'negative'))

final_dat_val$pred_class<- factor(final_dat_val$pred_class, c('positive', 'negative'))
final_dat_val$acc <- caret::confusionMatrix(table(final_dat_val$pred_class, final_dat_val$real))$overall[[1]]

final_dat_val$pred_class_opt <- as.factor(ifelse(final_dat_val$preds > unique(final_dat_val$optimal_thresh), 'positive', 'negative'))

final_dat_val$pred_class_opt<- factor(final_dat_val$pred_class_opt, c('positive', 'negative'))
final_dat_val$acc_opt <- caret::confusionMatrix(table(final_dat_val$pred_class_opt, final_dat_val$real))$overall[[1]]

acc_opt_val = round(unique(final_dat_val$acc_opt), 2)
acc_normal_val = round(unique(final_dat_val$acc), 2)
...
ConfusionMatrixInfo(data = final_dat, 
                    ...
                    other_title = paste0('optimal_thresh_test_', acc_opt, '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = final_dat, 
                    ...
                    actual = 'real', 
                    cutoff = 0.5,
                    other_title = paste0('normal_thresh_test_', acc_normal, '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = temp_450, 
                    ...
                    actual = 'real', 
                    cutoff = optimal_thresh,
                    other_title = paste0('optimal_thresh_null_450', '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = temp_450, 
                    ...
                    actual = 'real', 
                    cutoff = 0.5,
                    other_title = paste0('normal_thresh_null_450', '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = temp_850, 
                    ...
                    actual = 'real', 
                    cutoff = optimal_thresh,
                    other_title = paste0('optimal_thresh_null_850', '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = temp_850, 
                    ...
                    actual = 'real', 
                    cutoff = 0.5,
                    other_title = paste0('normal_thresh_null_850', '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)


# get confusion matrix function for plotting 
ConfusionMatrixInfo(data = final_dat_val, 
                    predict = 'preds', 
                    actual = 'real', 
                    cutoff = optimal_thresh,
                    other_title = paste0('optimal_thresh_valid_', acc_opt_val, '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)
...
ConfusionMatrixInfo(data = final_dat_val, 
                    ...
                    actual = 'real', 
                    cutoff = 0.5,
                    other_title = paste0('normal_thresh_valid_', acc_normal_val, '_',combat,'_' , method, '_', size, '_',  is_gen, '_',model_type),
                    get_plot = TRUE)


# get dataset of predictions and labels for both small and large data
pred_short <- prediction(final_dat_val$preds, final_dat_val$real)
...
plot(perf_s)
abline(a = 0, b =1)
temp <- round(roc(final_dat_val$real, final_dat_val$preds)[[9]], 2)
